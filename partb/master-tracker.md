...
| 2026-04-25 | HackerOne | RubyGems | `https://hackerone.com/rubygems?type=team` | `https://rubygems.org/` | YX / Codex | Program review, logged-in dashboard/settings source + network inspection, CSRF/form extraction, security-device and password-confirm flow review, OIDC/API key trusted-publisher route and related JS asset inspection | No finding | None | `partb/targets/2026-04-25-hackerone-rubygems/` |
| 2026-04-25 | HackerOne | SHEIN | `https://hackerone.com/shein?type=team` | `https://us.shein.com/` | YX / Codex | Program review, pre-auth source + BFF/API inspection, authenticated account/security/address review, extracted user-state endpoints (`userinfo_ugid`, `first_part_info`, `member_coupon_list`, `get_account_list`), route/source review for account/order/coupon/address flows, no cross-account object exposure or bypass confirmed | No finding | None | `partb/targets/2026-04-25-hackerone-shein/` |
| 2026-04-25 | HackerOne | Zooplus | `https://hackerone.com/zooplus?type=team` | `https://www.zooplus.com/` | YX / Codex | Program review, pre-auth source + route inspection, authenticated OIDC/session flow review, object/API inspection on account/cart/order/customer-data endpoints, invite page inline JWT/tokenPayload review, low-frequency cart/customer ID mutation tests, personal-info form/source inspection | No finding | None | `partb/targets/2026-04-25-hackerone-zooplus/` |
| 2026-04-25 | HackerOne | Vimeo | `https://hackerone.com/vimeo?type=team` | `https://vimeo.com/` | YX / Codex | Program review, authenticated survey/onboarding flow inspection, `_next/jwt` token review, source + Next data inspection, request-level tests on `api.vimeo.com/users/{id}` object endpoints, neighboring user ID checks on quota/capabilities/preferences/survey routes, no actionable exposure or bypass confirmed | No finding | None | `partb/targets/2026-04-25-hackerone-vimeo/` |
